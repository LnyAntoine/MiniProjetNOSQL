...
public class concatenateAndCreateSubset {
    ...
    private static final String RSQ_OUTPUT_SUBSET_DIR = "subsets/";
    ...
    private static final int MAX_QUERIES_IN_LITTLE_SUBSET = 10;
    private static final int MAX_QUERIES_IN_AVERAGE_SUBSET = 100;
    ...
    public static void main(String[] args) {
        try {
            ...
            try (java.io.BufferedWriter writer = new java.io.BufferedWriter(new java.io.FileWriter(LITTLE_SUBSET_FILE_PATH))) {
                subsetSize = Math.min(MAX_QUERIES_IN_LITTLE_SUBSET, allRefinedQueries.size());
                for (int i = 0; i < subsetSize; i++) {
                    writer.write(allRefinedQueries.get(
                            random.nextInt(allRefinedQueries.size())
                    ).toString());
                    writer.newLine();
                }
            }
            ...
            try (java.io.BufferedWriter writer = new java.io.BufferedWriter(new java.io.FileWriter(AVERAGE_SUBSET_FILE_PATH))) {
                subsetSize = Math.min(MAX_QUERIES_IN_AVERAGE_SUBSET, allRefinedQueries.size());
                for (int i = 0; i < subsetSize; i++) {
                    writer.write(allRefinedQueries.get(
                            random.nextInt(allRefinedQueries.size())
                    ).toString());
            ...
            try (java.io.BufferedWriter writer = new java.io.BufferedWriter(new java.io.FileWriter(BIG_SUBSET_FILE_PATH))) {
                subsetSize = Math.min(MAX_QUERIES_IN_BIG_SUBSET, allRefinedQueries.size());
                for (int i = 0; i < subsetSize; i++) {
                    writer.write(allRefinedQueries.get(
                            random.nextInt(allRefinedQueries.size())
                            ).toString());
